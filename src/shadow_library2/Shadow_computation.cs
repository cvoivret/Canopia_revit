namespace shadow_library2
{
    ...
    public class shadow_computation
    {
        ...
        public void ComputeShadowMultipleSunVector(Document doc, Reference winRef, Face face, XYZ sun_dir_Zmax, XYZ sun_dir_Zmin, List<string> log)
        {
            // COmpute shadow for one surface and multiple sun positions
            // Shoot some rays to store the results and use them for multiple extrusions

            // transformer le vecteur soleil dans le référentiel local de la fenetre
            // Faire une recherche depuis le centre de la fenetre avec une "grande" densité de rayons
            //    Chaque coins + centre : X rayons dans toutes les directions ? Dans la direction des vecteurs soleils ?
            // Strocker les solids (ou les reférences) par secteurs angulaires (azimtuh/elevation)
            // Rechercher les voisins dans les secteurs angulaires
            ...
            BoundingBoxUV bbuv = face.GetBoundingBox();
            /*XYZ corner1 = face.Evaluate(new UV(bbuv.Min[0], bbuv.Min[1])); //Umin,Vmin
            XYZ corner2 = face.Evaluate(new UV(bbuv.Min[0], bbuv.Max[1])); //Umin,Vmax
            XYZ corner3 = face.Evaluate(new UV(bbuv.Max[0], bbuv.Min[1])); //Umax,Vmin
            XYZ corner4 = face.Evaluate(new UV(bbuv.Max[0], bbuv.Max[1])); //Umax,Vmax
            double Zmin = Math.Min(corner1.Z, corner2.Z);
            double Zmax = Math.Max(corner1.Z, corner2.Z);

            log.Add(" corner  " + corner1);
            log.Add(" corner  " + corner2);
            log.Add(" corner  " + corner3);
            log.Add(" corner  " + corner4);
            */
            UV facecenter = new UV(0.5 * (bbuv.Min[0] + bbuv.Max[0]), 0.5 * (bbuv.Min[1] + bbuv.Max[1]));
            //Face solid angle discretization to shoot rays
            int Nx = 3, Ny = 3;
            ...
            foreach (ReferenceWithContext rc in referenceWithContexts2)
            {
                ...
                elementIds.Add(rc.GetReference().ElementId);
                //log.Add("+++++ intersection  "+rc.GetReference().ElementId);
            ...
            elapsedTime = String.Format("----extracting geom     : {0:N5}  ms     ", ts.TotalMilliseconds);
            log.Add(elapsedTime);
            return shadowing_solids;
            */
            return;
        }

        public Face ComputeShadow(Document doc, Face face, List<Solid> Candidates, XYZ extrusion_dir, double extrusion_dist, Transaction T, List<string> log)
        {
            ...
            Plane plane = Plane.CreateByNormalAndOrigin(face.ComputeNormal(facecenter), face.Evaluate(facecenter));
            Face ext_face = null;
            Solid s = null;
            ...
            try
            {
                s = GeometryCreationUtilities.CreateExtrusionGeometry(face.GetEdgesAsCurveLoops(), extrusion_dir, extrusion_dist);
            }
            catch (Exception e)
            {
            ...
            */
            ...
            foreach (Solid shad in Candidates)
            {
                ...
                {
                    log.Add("           Bolean Intersection failled (exception) " + e.ToString());
                }
            ...
            try
            {
                ExtrusionAnalyzer ea = ExtrusionAnalyzer.Create(union, plane, -extrusion_dir);
                ext_face = ea.GetExtrusionBase();
                //log.Add("           Extrusion retour =  " + ext_face.Area + " " + face.Area);

            }
            catch (Exception ex)
            {

                log.Add("           Extrusion retour failled ");
        ...
        public Face ComputeShadowByfaceunionfallback(Document doc, Face face, List<Solid> Candidates, XYZ extrusion_dir, double extrusion_dist, Transaction T, List<string> log)
        {
            //FamilyItemFactory factory = doc.FamilyCreate;
            //Form extruded = doc.FamilyCreate.NewExtrusionForm(true, ra, wall_normal.Multiply(10.0));
            Stopwatch sw = new Stopwatch();
            ...
            */
            ...
            try
            {
                // ExtrusionAnalyzer give only a contour, hole are filled
                ExtrusionAnalyzer ea = ExtrusionAnalyzer.Create(union, plane, -extrusion_dir);
                ext_face = ea.GetExtrusionBase();
                //log.Add("           Extrusion retour =  " + ext_face.Area + " " + face.Area);
                return ext_face;
            ...
            //////// FALL BACK : extrusion of each faces of the intersecting union, union of extrusion, extrudeanalyzer
            ...
            foreach (Face face1 in union.Faces)
            {
                if (face1 != null && face1.GetType() == typeof(PlanarFace))
                {
                    ...
                    if (normal.DotProduct(-extrusion_dir) > 0.00001 && face1.Area > 0.0001)
                    {
                        try
                        {
                            extrudedfaces.Add(GeometryCreationUtilities.CreateExtrusionGeometry(face1.GetEdgesAsCurveLoops(), -extrusion_dir, 5));
                            /*DirectShape ds = DirectShape.CreateElement(doc, new ElementId(BuiltInCategory.OST_GenericModel));
                            ds.ApplicationId = "Application id";
                            ds.ApplicationDataId = "Geometry object id";
            ...
            try
            {
                ExtrusionAnalyzer ea = ExtrusionAnalyzer.Create(union, plane, -extrusion_dir);
                ext_face = ea.GetExtrusionBase();
                //log.Add("           Second Extrusion retour =  " + ext_face.Area + " " + face.Area);

            }
            catch (Exception ex)
            {

                log.Add("           Second Extrusion retour failled ");
                //log.Add("           Exception : " + ex.ToString());
            }
