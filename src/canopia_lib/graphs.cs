...
namespace canopia_lib
{
    public class simpleGraph
    {
        Point[] vertices;
        int[,] edges;
        AdjacencyGraph<ElementId, Edge<ElementId>> graph;

        public simpleGraph()
        {
            graph= new AdjacencyGraph<ElementId, Edge<ElementId>>(false);
        }

        public void graphFromRoom(Room room)
        {
            // get boudary
            // insert each line as an edge with cost as line length
